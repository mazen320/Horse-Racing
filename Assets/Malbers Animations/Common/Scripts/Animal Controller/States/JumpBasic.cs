...
namespace MalbersAnimations.Controller
{
    ...
    public class JumpBasic : State
    {
        ...
        [Tooltip("Keep the Inertia from platforms or going forward to keep the forward momentum of the jump")]
        public BoolReference KeepPlatformInertia = new(true);
        ...
        public void ActivateJump()
        {
            ...
            {
                var currentSpeed = animal.HorizontalVelocity;

                //Draw the Jump Trajectory
                var JumpTrajectory = new List<Vector3>();
                var SimulatedPosition = animal.transform.position;
                var ExtraJumpHeight = (UpVector * activeJump.Height.Value);
                var deltaTime = Time.fixedDeltaTime;
                var gravityTime = activeJump.StartGravityTime;

                for (int i = gravityTime; i < gravityTime + 50; i++)
                {
                    var simulatedDeltaPos = Vector3.zero;
                    //Forward Movement
                    simulatedDeltaPos += deltaTime * ScaleFactor * currentSpeed;
                    //Height of the Jump
                    simulatedDeltaPos += deltaTime * ScaleFactor * ExtraJumpHeight;
                    //Apply Fake Gravity
                    simulatedDeltaPos += activeJump.GravityPower.Value * deltaTime * animal.StoredGravityVelocity(i);
                    SimulatedPosition += simulatedDeltaPos;
                    JumpTrajectory.Add(SimulatedPosition);
                ...
                {
                    Debug.DrawLine(JumpTrajectory[i], JumpTrajectory[i + 1], Color.yellow, 8);
                }
            }
        }
        ...
        public override void Activate()
        {
            ...
            if (animal.State_Get<Fall>() is Fall fallSt)
            {
                fallSt.LastPlatform = LastPlatform; //This is used to keep the inertia of the platform when you jump from it, if the platform is moving it will add that inertia to the jump
            }
        ...
        public override void OnStateMove(float deltaTime)
        {
            if (InCoreAnimation)
            {
                ...
                if (LastPlatform)
                {
                    var PlatformDelta = LastPlatform.position - animal.Last_Platform_Pos;
                    animal.AdditivePosition += PlatformDelta;
                    animal.Last_Platform_Pos = LastPlatform.position;
                }
                ...
                if (ActivateJumpLogic)
                {
                    ...
                    if (activeJump.ForwardPush > 0)
                        animal.AdditivePosition += activeJump.ForwardPush * deltaTime * ScaleFactor * Vector3.ProjectOnPlane(animal.Forward, animal.UpVector);     //Forward Movement
                    ...
                    if (KeepPlatformInertia) animal.AdditivePosition += PlatFormInertia;
                    ...
                    {
                        animal.AdditivePosition += activeJump.GravityPower.Value * deltaTime * animal.StoredGravityVelocity(animal.GravityTime);
        ...
        internal override void Reset()
        {
            ...
            sleepFromState = new() { IDs = new() { MTools.GetInstance<StateID>("Fly") } };
            sleepFromMode = new() { IDs = new() { MTools.GetInstance<ModeID>("Action"), MTools.GetInstance<ModeID>("Attack1") } };
