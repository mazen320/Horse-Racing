...
namespace MalbersAnimations.Controller
{
    public abstract class State : ScriptableObject
    {
        ...
        public IDListCheck<StateID> queueFrom = new();
        ...
        public IDListCheck<StateID> resetFrom = new();

        [Tooltip(" If A Stance is active, and is one of one on the List. The state cannot be activated")]
        public IDListCheck<StanceID> sleepFromStance = new();

        [Tooltip("Which stances are allowed during this State. Leave empty to include all")]
        public IDListCheck<StanceID> allowedStances;

        [Tooltip("Pause the Editor when the state is active")]
        public bool pauseOnActivate = false;

        /// <summary>  Has some Reset From States  </summary>
        public bool HasResetFrom => !resetFrom.Empty;

        [HideInInspector] public List<ModeID> SleepFromMode = new();            //OLD WAY it will be replaced by the IDList. In future updates, this will be removed   
        [HideInInspector] public List<StateID> SleepFromState = new();          //OLD WAY it will be replaced by the IDList. In future updates, this will be removed   
        [HideInInspector] public List<ModeID> DisableModes = new();             //OLD WAY it will be replaced by the IDList. In future updates, this will be removed   
        [HideInInspector] public List<StateID> QueueFrom = new();               //OLD WAY it will be replaced by the IDList. In future updates, this will be removed   
        [HideInInspector] public List<StateID> ResetFrom = new();               //OLD WAY it will be replaced by the IDList. In future updates, this will be removed   
        [HideInInspector] public List<StanceID> SleepFromStance = new();        //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
        [HideInInspector] public List<StanceID> stances = new();                //OLD WAY IT WILL BE REPLACED BY THE IDLIST. In future updates, this will be removed
        ...
        internal bool ValidStance(StanceID currentStance) => allowedStances.Check(currentStance);
        ...
        public EnterExitIDEvent<StateID> EnterExitEvent;
        ...
        public void SetPersistent(bool value) => IsPersistent = value;
       ...
       // public bool PendingExit { get; set; }
        ...
        public void AwakeState(MAnimal mAnimal)
        {
            ...
            if (allowedStances == null || allowedStances.Empty) allowedStances = new(stances); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (sleepFromStance == null || sleepFromStance.Empty) sleepFromStance = new(SleepFromStance); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (queueFrom == null || queueFrom.Empty) queueFrom = new(QueueFrom); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (resetFrom == null || resetFrom.Empty) resetFrom = new(ResetFrom); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (sleepFromState == null || sleepFromState.Empty) sleepFromState = new(SleepFromState); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (sleepFromMode == null || sleepFromMode.Empty) sleepFromMode = new(SleepFromMode); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
            if (disableModes == null || disableModes.Empty) disableModes = new(DisableModes); //OLD WAY it will be replaced by the IDList. In future updates, this will be removed
        }
...
#if UNITY_EDITOR
            if (pauseOnActivate) Debug.Break();
#endif
        }
        ...
        internal void ActivatebyInput(bool value)
        {
            ...
            )
            {
                ...
                if (resetInputOnFailed)
                {
                    ...
                    //THIS IS CAUSING AN ISSUE WHEN MULTIPLE STATES ARE SUBSCRIBED TO THE SAME STATE - Bug (fixed on MInput Link )
                    if (!animal.InputSource.IsUnityRefNull())
                        animal.InputSource.ResetInput(Input);
                }
            }
        ...
        internal void ExitByInput(bool exitValue)
        {
            ...
            //Only Exit if we are the active state
        ...
        internal void NewStateActivated(StateID stateID)
        {
            ...
            if (OnHoldByReset && resetFrom.Check(stateID))
            {
                OnHoldByReset = false;

                Debugging("Reset from ResetFrom List");
            }
        ...
        public void Debugging(string value, string color = "white")
        {
            ...
            if (m_debug && animal.debugStates)
                MDebug.Log($"<B>[{animal.name}]</B> → <B> <color=green>[{this.GetType().Name}]</color></B> → <color={color}>{value}</color>", this);
#endif
        }
        #endregion 
        ...
        /// <summary>When creating the states for the first time, it will add to the Animal a new Speed Set EDITOR ONLY</summary>
        ...
        /// <summary>Called the Animal Grounded Platform has changed</summary>
        public virtual void OnPlatformChanged(Transform newPlatform) { }
        ...
        {
            if (!allowedStances.Valid && stances != null && stances.Count > 0)
            {
                allowedStances = new(stances);
                stances = null;
            }

            if (!sleepFromStance.Valid && SleepFromStance != null && SleepFromStance.Count > 0)
            {
                sleepFromStance = new(SleepFromStance);
                SleepFromStance = null;
            }


            if (!queueFrom.Valid && QueueFrom != null && QueueFrom.Count > 0)
            {
                queueFrom = new(QueueFrom);
                QueueFrom = null;
            }

            if (!resetFrom.Valid && ResetFrom != null && ResetFrom.Count > 0)
            {
                resetFrom = new(ResetFrom);
                ResetFrom = null;
            }

            if (!sleepFromState.Valid && SleepFromState != null && SleepFromState.Count > 0)
            {
                sleepFromState = new(SleepFromState);
                SleepFromState = null;
            }

            if (!sleepFromMode.Valid && SleepFromMode != null && SleepFromMode.Count > 0)
            {
                sleepFromMode = new(SleepFromMode);
                SleepFromMode = null;
            }

            if (!disableModes.Valid && DisableModes != null && DisableModes.Count > 0)
            {
                disableModes = new(DisableModes);
                DisableModes = null;
            }
        }
    }
    ...
    public class StateEd : Editor
    {
        SerializedProperty
           ID, Input, ExitInput, enterInputValue, exitInputValue,
            ...
            IncludeChildren, AllowExitAnimation,
            //  IncludeSleepState,
            Duration, S_Speed_List,
           ...
           SleepFromState, SleepFromMode, noModes, DisableModes,
            ...
            , stances, allowedStances, SleepFromStance;
        ...
        protected virtual void OnEnable()
        {
            ...
            pauseOnActivate = serializedObject.FindProperty("pauseOnActivate");

            stances = serializedObject.FindProperty("stances");
            allowedStances = serializedObject.FindProperty("allowedStances");
        ...
        private void ShowLimits()
        {
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                EditorGUILayout.PropertyField(SleepFromState);
                EditorGUILayout.PropertyField(QueueFrom);
                EditorGUILayout.PropertyField(ResetFrom);
            }
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                EditorGUILayout.PropertyField(allowedStances);
                EditorGUI.indentLevel--;
            }
        }
        ...
        protected virtual void ShowState()
        {
...

            {
                ...
                EditorGUILayout.PropertyField(pauseOnActivate);
            }
        }
    }
    ...
    public class AnimalModifierDrawer : UnityEditor.PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(UnityEditor.SerializedProperty property, GUIContent label)
        {
            ...
            float lines = (activeProperties + 2) / 3 + 1;
