...
namespace MalbersAnimations.Controller
{
    ...
    public class Locomotion : State
    {
        ...
        public struct WallStopProfiles
        {
            ...
            [Tooltip("Forward ray length for wall detection at this speed")]
        ...
        public override void Activate()
        {
            ...
            //When entering Locomotion from Idle or if the Smooth Vertical is on, set the Enter Status to the current Speed Index to play the correct animation and not always the Walk one
            if (!animal.UseSmoothVertical || animal.RawInputAxis.magnitude > 0.7f)
        ...
        private void Wall_Stop()
        {
            if (WallStop && currentProfile.SpeedIndex != 0 && MovementRaw.z > 0) //Find a wall if we are doing Wall Stopping
            {
                ...
                MDebug.DrawRay(MainPivotPoint, animal.Forward * Length, Color.yellow);
                ...
                {
                    MDebug.DrawRay(MainPivotPoint, animal.Forward * Length, DebugColor);
                    if (WallHit)
                    {
                        WallHit = null;
        ...
        private void CheckCurrentWallProfile(int SpeedIndex)
        {
            if (WallStop)
            {
                currentProfile.Empty(); // ← reset first, then search
                ...
                //debug
                Debugging($"Current Wall Stop Profile: {currentProfile.SpeedIndex} - {currentProfile.RayLength}");
            }
        }
        ...
        private void Anti_Fall()
        {
            if (AntiFall)
            {
                ...
                if (animal.UseCameraInput)
                {
                    ...
                    bool BlockForward =
                        Physics.RaycastNonAlloc(Center, Dir, hits, RayMultiplier, GroundLayer, IgnoreTrigger) == 0 ||
                        Physics.RaycastNonAlloc(Left, Dir, hits, RayMultiplier, GroundLayer, IgnoreTrigger) == 0 ||
                        Physics.RaycastNonAlloc(Right, Dir, hits, RayMultiplier, GroundLayer, IgnoreTrigger) == 0;
