                                ...
                                if (Element.isExpanded)
                                {
                                    using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                                    {
                                        using (var aa = new EditorGUI.ChangeCheckScope())
                                        {
                                            ...
                                            if (aa.changed)
                                            {
                                                current.intValue = current.intValue - 1; // Decrease by 1 because the ChangeMaterial will increase it by 1 to get the correct material index

                                                serializedObject.ApplyModifiedProperties();
                                                ToggleButton(list.index);
                                                Undo.RecordObject(target, "Change Current Index");
                                            }
                                        }
