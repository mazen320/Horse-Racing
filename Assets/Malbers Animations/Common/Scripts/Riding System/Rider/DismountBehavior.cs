    ...
    public class DismountBehavior : StateMachineBehaviour
    {
        ...
        override public void OnStateMove(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if (rider.Montura)  //Stop the Mountura from walking forward when Dismounting
            {
                ...
                {
                    if (hit.collider.transform.SameHierarchy(rider.transform))
                    {
                        Debug.LogWarning($"Dismounting: Hit {hit.collider.name} detected on the same hierarchy as the rider. Make sure the ground Layer  is not the rider or its children.");
                        return;
                    }
