...
namespace MalbersAnimations.IK
{
    ...
    public class IKTargetDir : IKProcessor
    {
        ...
        public override void OnAnimatorIK(IKSet IKSet, Animator anim, int index, float weight, int LayerIndex)
        {
            ...
            if (IKSet.Targets.Length != 0)
            {
                ...
                if (targets != null)
                {
                    ...
                    if (enableRotation && targetPosition != bodyPart.position) // If the target position is the default position don't rotate
                    {
