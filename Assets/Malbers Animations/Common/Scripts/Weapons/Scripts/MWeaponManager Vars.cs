...
namespace MalbersAnimations
{
    ...
    public partial class MWeaponManager
    {
        ...
        public bool IsReloading => Weapon != null && Weapon.IsReloading; //MWC: bug fix — null guard added
        ...
        // MWC: legacy backing fields — FormerlySerializedAs loads old YAML data into these; OnValidate migrates them into the new IDList fields
        [SerializeField, HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("DisableModes")]
        private List<ModeID> m_DisableModes_Legacy = new();
        [SerializeField, HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("ExitOnModes")]
        private List<ModeID> m_ExitOnModes_Legacy = new();
        [SerializeField, HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("DisableStates")]
        private List<StateID> m_DisableStates_Legacy = new();
        [SerializeField, HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("ExitOnState")]
        private List<StateID> m_ExitOnState_Legacy = new();
        [SerializeField, HideInInspector, UnityEngine.Serialization.FormerlySerializedAs("DropWeapon")]
        private List<StateID> m_DropWeapon_Legacy = new();

        [Tooltip("Disable these modes when a weapon is equipped")]
        public IDList<ModeID> DisableModes = new();

        [Tooltip("Unequip Weapons If any of these modes are Activated")]
        public IDList<ModeID> ExitOnModes = new();

        [Tooltip("Disable these States when a weapon is equipped")]
        public IDList<StateID> DisableStates = new();

        [Tooltip("Unequip Weapons If any of these states are Activated")]
        public IDList<StateID> ExitOnState = new();

        [Tooltip("Drop Weapons If any of these states are Activated")]
        public IDList<StateID> DropWeapon = new();
        ...
        public int ComboBranch => comboManager != null ? comboManager.Branch : 0; //MWC: bug fix — null guard added
        ...
        [Tooltip("Custom IK Profiles for specific weapons. This will override the Aim IK values on the weapon (if is left handed)")]
        public List<IDPair<WeaponID, IKProfile>> CustomIKWeaponsLeft;
        [Tooltip("Custom IK Profiles for specific weapons. This will override the Aim IK values on the weapon (if is right handed)")]
        public List<IDPair<WeaponID, IKProfile>> CustomIKWeaponsRight;

        public float CharacterScale = 1f; //Used to scale the IK values for the weapons

        public IKProfile CurrentIKProfile { get; set; }
