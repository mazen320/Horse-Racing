...
namespace MalbersAnimations
{
    ...
    public partial class MWeaponManager
    {
        ...
        public virtual void Holster_Previous()
        {
            ActiveHolsterIndex = (ActiveHolsterIndex - 1 + holsters.Count) % holsters.Count; //MWC: also fixed negative-modulo bug (e.g. index 0-1 = -1)
            ActiveHolster = holsters[ActiveHolsterIndex];
            Draw_Weapon();
        }

        /// <summary>Equip the previous holster weapon</summary>
        [System.Obsolete("Use Holster_Previous (corrected spelling)")] //MWC: typo fix — kept for backward compatibility
        public virtual void Holster_Previus() => Holster_Previous();
        ...
        /// <summary>Drop the Current Equipped weapon</summary> 
        public virtual void Drop_Weapon(Vector3 position)
        {
            ...
            if (Weapon)
            {
                ...
                Weapon.OnDroppedBy.Invoke(gameObject);
            }
        }

        public virtual void Drop_Weapon(Transform Point, bool ResetLocal = false)
        {
            var WeaponGO = Weapon != null ? Weapon.gameObject : null;
            ...
            if (Point != null)
            {
                if (ResetLocal) WeaponGO.transform.ResetLocal();
            }
        }
        ...
        private IEnumerator I_SwappingWeapons;
        ...
        protected virtual void Holster_AddWeapon(Holster holster, MWeapon weap)
        {
            if (holster.Weapon) //Meaning there's a OLD weapon already on the holster
            {
                if (holster.Weapon.IsCollectable != null)
                {
                    ...
                    else // if Malbers Inventory doesn't exist
                    {
                        ...
                        holster.Weapon.OnDroppedBy.Invoke(gameObject);
                    }
                }
                else
                {
            ...
            {
                //This is already done by the Item Picked so Don't call it twice
                //if (holster.Weapon.IsCollectable != null)
