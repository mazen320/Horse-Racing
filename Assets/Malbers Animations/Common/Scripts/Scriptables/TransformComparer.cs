...
namespace MalbersAnimations.Scriptables
{
    ...
    public class TransformComparer : VarListener
    {
        ...
        {
            value.Value = target != null ? target.transform : null;
            Invoke();
        }
        ...
        public void SetValue(Object target)
        {
            ...
            else if (target is GameObject go) SetTarget(go);
            else if (target is Component comp) SetTarget(comp);
            else
                Debug.LogWarning($"<B>{name}:</B> Target must be a GameObject or Component. Current target is of type {target.GetType()}", this);
        }
