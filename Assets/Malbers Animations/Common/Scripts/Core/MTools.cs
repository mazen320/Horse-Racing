...
namespace MalbersAnimations
{
    ...
    {
        public static readonly string OnFocusedPath = "Assets/Malbers Animations/Common/Prefabs/UI/OnFocused.prefab";

        public static string CompareToString(ComparerNumber Compare)
        {
        ...
        public static string CompareToString(ComparerInt Compare) => CompareToString((ComparerNumber)Compare);
        ...
        public static Vector3 CalculateNormalFrom2Points(Vector3 centerPoint, Vector3 forwardPoint, Vector3 UpVector)
        {
            Vector3 direction = (forwardPoint - centerPoint).normalized;
        ...
        #region Types
        public static List<Type> GetAllTypes<T>() => MalbersReflection.GetAllTypes<T>();
        public static List<Type> GetAllTypes(Type type) => MalbersReflection.GetAllTypes(type);
        #endregion

        #region Find References
        public static Camera FindMainCamera()
        {
            var MainCamera = Camera.main != null ? Camera.main : GameObject.FindAnyObjectByType<Camera>();
            return MainCamera;
        }
        #endregion
        ...
        {
            XmlSerializer xml = new(typeof(T));
            StringReader reader = new(toDeserialize);
        ...
        public static IEnumerator AlignLookAtTransformDirection(Transform t1, Vector3 direction, float time, float angleOffset = 0, AnimationCurve curve = null)
        {
            float elapsedTime = 0;
            var wait = new WaitForFixedUpdate();
...
#if UNITY_EDITOR
        #region Styles      
        public static GUIStyle StyleDarkGray => Style(MDarkGray);
        public static GUIStyle StyleGray => Style(MGray);
        ...
        public static Color MDarkGray = new(0.20f, 0.20f, 0.25f, 0.2f);
        public static Color MGray = new(0.35f, 0.5f, 0.7f, 0.2f);
        ...
        public static void DrawScriptableObject(ScriptableObject serializedObject, bool showscript = true, int skip = 0)
        {
            ...
            SerializedObject serialied_element = new(serializedObject);
        ...
        /// <summary>
        /// Adds the OnFocused Prefab to a Transform
        /// </summary>
        /// <param name="transform"></param>
        /// <param name="OnEnter"></param>
        /// <param name="OnExit"></param>
        public static void AddOnFocusedPrefab(Transform transform, UnityEventBase OnEnter, UnityEventBase OnExit)
        {
#if UNITY_EDITOR
            GameObject OnFocusedP = AssetDatabase.LoadAssetAtPath(OnFocusedPath, typeof(GameObject)) as GameObject;

            if (OnFocusedP)
            {
                OnFocusedP = (GameObject)PrefabUtility.InstantiatePrefab(OnFocusedP);
                OnFocusedP.transform.SetParent(transform);
                OnFocusedP.transform.ResetLocal();

                UnityEditor.Events.UnityEventTools.AddBoolPersistentListener(OnEnter, OnFocusedP.SetActive, true);
                UnityEditor.Events.UnityEventTools.AddBoolPersistentListener(OnExit, OnFocusedP.SetActive, false);
            }
#endif
        }
