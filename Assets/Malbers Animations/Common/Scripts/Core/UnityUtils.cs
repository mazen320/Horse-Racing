...
namespace MalbersAnimations
{
    ...
    public class UnityUtils : MonoBehaviour
    {
        ...
        {
            StopAllCoroutines(); // Stop all coroutines when the component is disabled to prevent unintended behavior
        }
        ...
        public void Position_Reset(GameObjectVar go) => go.transform.localPosition = Vector3.zero;

        public virtual void GameObject_Toggle(GameObject go) => go.SetActive(!go.activeSelf);
        public virtual void GameObject_Toggle(GameObjectVar go) => go.Value?.SetActive(!go.Value.activeSelf);
        ...
        /// <summary>Ugly way to stop all audio sources on the scenes</summary>
        public virtual void PauseAllAudio(bool pause)
        {
            if (!enabled) return;

#if UNITY_6000_4_OR_NEWER
            audios ??= FindObjectsByType<AudioSource>(FindObjectsInactive.Exclude); //MWC: FindObjectsSortMode obsolete in Unity 6000.4+
#else
