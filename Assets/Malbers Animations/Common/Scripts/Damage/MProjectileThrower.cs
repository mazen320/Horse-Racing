...
namespace MalbersAnimations.Weapons
{
    ...
    public class MProjectileThrower : MonoBehaviour, IThrower, IAnimatorListener
    {
        ...
        [Min(0), Tooltip("Debug Size for the Gizmos when Debug is enabled")]
        ...
        public GameObject UserGo { get => m_Owner.Value; set => m_Owner.Value = value; }
        ...
        public bool CalculateTrajectory
        {
            ...
            set
            {
                ...
                //Debug.Log($"CalculateTrajectory : [{value}]");
            }
        }
        ...
        {
            if (UserGo == null) UserGo = transform.FindObjectCore().gameObject;
        ...
        public virtual void Fire()
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                if (debug) Debug.Log($"<B>[{UserGo.name}]</B> is Firing a <B>[{Projectile.name}]</B> with a <B>[{Power}]</B> Force", this);
            }
        }
        ...
        void Prepare_Projectile(GameObject p)
        {
            if (spreadAngle > 0)
            {
                Velocity = Velocity.SpreadDirectionByAngle(spreadAngle, spreadMult);
            }
            ...
            if (p.TryGetComponent<IProjectile>(out var projectile))
            {
                ...
                projectile.Prepare(UserGo, Gravity, Velocity, Layer, TriggerInteraction, ProjectilePool);
                //Pool - End  
        ...
        void OnDrawGizmos()
        {
            if (debug && AimOrigin)
            {
                ...
                Handles.ArrowHandleCap(0, m_AimOrigin.position, m_AimOrigin.rotation, Power * 0.01f * debugSize, EventType.Repaint);
            }
        }
#endif
    }
    ...
    public class MProjectileThrowerEditor : Editor
    {
        private SerializedProperty m_Projectile, m_Force, m_angle, gravity, OnFire,
            ...
            amount, FireOnStart, debug, debugSize,
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                if (m_Projectile.isExpanded = MalbersEditor.Foldout(m_Projectile.isExpanded, "Projectile"))
                {
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        EditorGUILayout.PropertyField(debugSize, GUIContent.none, GUILayout.Width(40));
