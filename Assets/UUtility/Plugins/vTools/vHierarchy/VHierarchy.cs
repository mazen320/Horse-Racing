    ...
    public static class VHierarchy
    {
        static void RowGUI(EntityId entityId, Rect rowRect)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < EditorSceneManager.sceneCount; i++)
                    if (EditorSceneManager.GetSceneAt(i).GetHashCode() == entityId)
                        iScene = i;
        ...
        static void GameObjectRowGUI(GameObject go, Rect rowRect)
        {
            ...
            void hydrateData()
            {
                void scene()
                {
                    ...
                    foreach (var kvp in sceneData.goDatasByGlobalId)
                    {
                        ...
                        var iid = GlobalObjectId.GlobalObjectIdentifierToEntityIdSlow(gid);

                        if (iid != EntityId.None)
                            sceneData.goDatasByInstanceId[(int)EntityId.ToULong(iid)] = kvp.Value;
                        else
                ...
                void go_()
                {
                    sceneData.goDatasByInstanceId.TryGetValue((int)EntityId.ToULong(go.GetEntityId()), out goData);
                }
            ...
            void ensureGoDataExists()
            {
                ...
                sceneData.goDatasByGlobalId[GlobalObjectId.GetGlobalObjectIdSlow(go.GetEntityId()).ToString()] = goData;
                sceneData.goDatasByInstanceId[(int)EntityId.ToULong(go.GetEntityId())] = goData;
        ...
        static void SceneRowGUI(Scene scene, Rect rowRect)
        {
            ...
            void collapseEverything()
            {
                ...
                expandQueue_toAnimate = expandedRoots.Select(r => new ExpandQueueEntry(r.GetEntityId(), false))
                                                     .OrderBy(r => VisibleRowIndex(r.entityId)).ToList();
        ...
        static void CheckShortcuts()
        {
            ...
            void toggleExpanded()
            {
                ...
                SetExpandedWithAnimation(hoveredGo.GetEntityId(), !expandedIds.Contains(hoveredGo.GetEntityId()));
                EditorApplication.RepaintHierarchyWindow();
            ...
            void collapseEverythingElse()
            {
                ...
                expandQueue_toAnimate = toCollapse.Select(r => new ExpandQueueEntry(r.GetEntityId(), false))
                                                         .Append(new ExpandQueueEntry(hoveredGo.GetEntityId(), true))
                                                         .OrderBy(r => VisibleRowIndex(r.entityId)).ToList();
        ...
        static void UpdateExpandQueue()
        {
            ...
            if (!expandQueue_toAnimate.Any())
            {
                ...
                foreach (var r in expandQueue_toCollapseAfterAnimation)
                    SetExpanded(r.GetEntityId(), false);
            ...
            var entityId = expandQueue_toAnimate.First().entityId;
            var expand = expandQueue_toAnimate.First().expand;

            if (expandedIds.Contains(entityId) != expand)
                SetExpandedWithAnimation(entityId, expand);
        ...
        struct ExpandQueueEntry { public EntityId entityId; public bool expand; public ExpandQueueEntry(EntityId entityId, bool expand) { this.entityId = entityId; this.expand = expand; } }

        static void UpdateExpandedIdsList()
        {
            if (!Application.isPlaying)
                expandedIds = hierarchyWindow?.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeViewState").GetPropertyValue<List<EntityId>>("expandedIDs");

            EditorApplication.delayCall -= UpdateExpandedIdsList;
            EditorApplication.delayCall += UpdateExpandedIdsList;

        }
        static List<EntityId> expandedIds;

        static void SetExpandedWithAnimation(EntityId entityId, bool expanded) => hierarchyWindow.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeView").InvokeMethod("ChangeFoldingForSingleItem", entityId, expanded);
        static void SetExpanded(EntityId entityId, bool expanded) => hierarchyWindow.InvokeMethod("SetExpanded", entityId, expanded);
        static bool IsExpanded(GameObject go) => expandedIds != null && expandedIds.Contains(go.GetEntityId());
        ...
        static int VisibleRowIndex(EntityId entityId) => hierarchyWindow.GetFieldValue("m_SceneHierarchy").GetFieldValue("m_TreeView").GetPropertyValue("data").InvokeMethod<int>("GetRow", entityId);
